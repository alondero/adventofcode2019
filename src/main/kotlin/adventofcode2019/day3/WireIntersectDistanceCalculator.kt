...
typealias Mover = (Coordinate) -> Coordinate
typealias SignalDelay = Int

fun calculateClosestIntersection(wireA: String, wireB: String) = coordinateMapFor(wireA)
    .intersect(coordinateMapFor(wireB))
    .map { manhattanDistanceFor(it) }
    .min()

private fun coordinateMapFor(wire: String): Collection<Coordinate> = coordinateMapAndSignalDelayFor(wire).map { it.key }

private fun coordinateMapAndSignalDelayFor(wire: String): Map<Coordinate, SignalDelay> {
    var currentCoordinate = Coordinate(0, 0)
    var currentSignalDelay = 0

    return wire.split(",").map { instruction ->
        val directionMove = moverFor(instruction[0])
        val numMoves = instruction.substring(1).toInt()
        (1..numMoves).map {
            (directionMove(currentCoordinate) to ++currentSignalDelay).also { currentCoordinate = it.first }
        }
    }.flatten().toMap()
}

private fun manhattanDistanceFor(coord: Coordinate) = abs(coord.first) + abs(coord.second)

// IntelliJ is getting itself mixed up if I don't include explicit parameters
private fun moverFor(direction: Char): Mover = when (direction) {
    'U' -> { coord -> Coordinate(coord.first + 1, coord.second) }
    'D' -> { coord -> Coordinate(coord.first - 1, coord.second) }
    'R' -> { coord -> Coordinate(coord.first, coord.second + 1) }
    'L' -> { coord -> Coordinate(coord.first, coord.second - 1) }
    else -> { it -> it }
}
